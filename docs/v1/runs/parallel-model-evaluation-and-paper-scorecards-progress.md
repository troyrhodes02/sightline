...
- **SIG-106 (PME-5, three portfolios)** ✅ DONE — PR #110, branch `wtrhodesdev/sig-106-pme-5-three-paper-portfolios-per-portfolio-cycles-scorecards` (branched off SIG-104). Jest 1005/1005, typecheck/build/format clean. Per-portfolio cycle iteration in `src/lib/pipeline/paper-cycle.ts` via new `src/lib/paper/portfolios.ts`; scorecard read `src/lib/paper/scorecards.ts` (+ `PortfolioScorecardDto` in dto/model-eval.ts). Hybrid attribution permanent (`sourceModelVersion` stamped at open, never in update path — structural test). Additive `recalibrationByVersion` on planCycle keeps D4. Linear → In Progress.
...
- **SIG-107 (PME-6, Paper Bot reorg)** ✅ DONE — PR #112, branch `wtrhodesdev/sig-107-pme-6-paper-bot-reorg-performanceactivitysettings-model` (off SIG-105). Jest 1042/1042, typecheck/build/format clean; e2e skip-gated locally (runs on Vercel preview). Autonomy→Paper Bot (Performance/Activity/Settings); legacy routes 308-redirect; `POST /api/model-selection` (`applyModelSelection`) is the ONLY config writer — resets `portfolioStartedAt` on the newly-active config's portfolio (D2), stamps activeConfigChangedAt, records PaperControlEvent, one $transaction; readiness counts from that portfolio (D2); Dry Run screen/page/route/tab deleted, `runDryRun`/`planCycle` kept test-callable (D7). Structural invariant tests: only model-selection.ts writes ModelSelection; not renamed "Preview". Added `SIMULATION_SUPPORTED_STATS` (yardage+receptions; TDs Sim n/a). Linear → In Progress.
  - **Deferred:** per-portfolio Resume/Force-override render in the banner but act via the existing single-campaign routes (dedicated per-portfolio override route out of scope) — note for report.
- **SIG-108 (PME-7, viewer track-record block)** ⏳ next (LAST ticket) — branches off SIG-107's branch.
